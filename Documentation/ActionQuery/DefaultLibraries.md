...
**ForceTextResponse([contentType])** - Configures text response builder for the result of the execution. Should be used carefully, usually combined with OverrideResponseData and BailOut, because text response will not serialize structured data usually generated by a nodeset. The Result.Data should be a string or at least something that makes sense when converted to string. The response is text/plain by default, but can be set to anything by passing a string parameter with the desired MIME type.

**ForceJSONResponse()** - Configures JSON response for the result (application/json). All the Result.Data is serialized as plain JSON in this case. This will work for most of the accumulated results, yet some values can be non-serializeable and one has to have that in mind. BailOut may ot may not be useful depending on the scenario, because JSON serialization can accommodate almost any typical results, so bailing out is needed only if further processing is not needed by business logic reasons.

**OverrideResponseData(data)** - replaces the Result.Data with the content of the parameter. In almost any case this is needed it will involve bailing out (BailOut) and possibli ForceXXXResponse function also, because replacing the accumulated result with explicit data usually targets response that does not fit the typical nodeset results.

**BailOut()** - finishes the nodeset execution after the script calling it finishes executing. which makes it used as `BalOut(), halt` - typical especially if BailOut is called conditionally in the middle of the script somewhere.

#### Miscellaneous

**DictFromParameters(paramliststring)** - this function returns a Dict with the values of the listed parameters, keyed with their names. The list is supplied in a string parameter expected to contain comma separated list of parameter names. The function is mostly useful for scripts calling other nodes with composed parameters (to transfer some parameters in the call without need to construct dictionary explicitly)